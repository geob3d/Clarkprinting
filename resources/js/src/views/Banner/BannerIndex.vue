<template>
  ...
  <div id="data-list-list-view" class="data-list-container">
   ...
   <!--  <data-view-sidebar :isSidebarActive="addNewDataSidebar" @closeSidebar="toggleDataSidebar" :data="sidebarData" /> -->
    ...
    <vs-table multiple v-model="selected" @selected="handleSelected" :data="banners">
        ...
        <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            ...
            <!-- ACTION - DROPDOWN -->
             <div class="centerx ">
                ...
                <vs-popup class="holamundo"  title="Add New Banner" :active.sync="popupActivo">
                    <banner-add />
                </vs-popup>
            </div>
        ...
        <template slot="thead">
            <vs-th>ID</vs-th>
            <vs-th sort-key="company_id">Company Name</vs-th>
            <vs-th>Chain No.</vs-th>
        ...
        <template slot-scope="{data}">
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                ...
                <vs-td :data="data[indextr].companies.name">
                    {{data[indextr].companies.company_code}}
                </vs-td>

                <vs-td :data="data[indextr].chain_no">
                    {{data[indextr].chain_no}}
                </vs-td>
                ...
                <vs-td class="whitespace-no-wrap">
                  ...
                  <router-link :to="{name: 'EditBanner', params: { id: tr.id }}">
                    <feather-icon icon="EditIcon" svgClasses="w-5 h-5 hover:text-primary stroke-current"  />
                  </router-link>

                    <feather-icon icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.prevent="deletebanners(tr.id)" />
                 </vs-td>
...
import BannerAdd from './BannerAddSideBar'
...
export default {
    ...
    methods: {
        ...
        handleSelected(tr) {
            this.$vs.notify({
                title: `Selected ${tr.chain_no}`,
                text: `Banner: ${tr.name}`
            })
        },

        deletebanners(id){
          if (confirm(`Are you sure you want to delete banners ${id}?`)){
          let uri = `/api/banner/${id}`;
          axios
          .delete(uri)
          .then(response => {});
          this.getBanners();
